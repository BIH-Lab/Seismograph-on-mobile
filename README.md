...
- 파일 로드 후 **파형**이 표시되고, 파형을 드래그·핀치하여 구간 탐색
- 스펙트로그램은 파형 뷰포트를 따라 자동 동기화
...
**그래프 PNG 내보내기:**
- 각 그래프 하단 **↓ PNG** 버튼 → ObsPy 스타일 메타데이터 헤더 포함 이미지 저장
- 파일명 형식: `seismo_{type}_{station}_{date}.png`
- 헤더에 관측소 ID·샘플레이트·날짜 자동 삽입
...
| 04 | 주시곡선 (굴절법 탐사) | ✅ 완료 | 여러 스마트폰 CSV 로드 → 시간 동기화 → 초동 픽킹 → V₁/V₂ 속도·하부층 두께·교차거리 계산 |
| 05 | 진원 찾기 | 준비중 | P파·S파 도달 시간 및 GPS 위치로 진원지 역산 |

---

### Activity 4 (주시곡선 — 굴절법 탐사)

여러 스마트폰으로 동시에 기록한 CSV 파일을 분석해 지층 구조(표층 P파 속도 V₁, 하부층 속도 V₂, 층 두께 h, 교차거리 xc)를 계산합니다.

**3단계 워크플로우:**

**Step 1 — 파일 로드 & 거리 설정**
- Activity 2 CSV 파일 여러 개를 동시에 로드 (드래그 앤 드롭 또는 파일 선택)
- GPS 좌표가 있으면 진원 스테이션 기준으로 거리 자동 계산 (Haversine), 없으면 수동 입력

**Step 2 — 시간 동기화 (Record Section)**
- X=거리(m), Y=시간(↓) 좌표계의 wiggle 트레이스 시각화
- 빨간 기준선 = 발진(충격) 시각 t=0; 드래그로 이동
- 각 트레이스에서 충격 순간 클릭 → 그 시각이 기준선으로 스냅 (클릭-to-스냅 시계 동기화)
- 줌인/아웃 + 핀치 줌, 정밀 조정 패널 (ms 단위 입력)

**Step 3 — 초동 픽킹 & 주시곡선**
- 각 스테이션 파형 카드에서 P파 초동 클릭
- 카드별 줌인/아웃 + 핀치 줌, ±진폭 스케일 표시
- 주시곡선 자동 회귀 모드: 슬라이더로 직접파·굴절파 경계 조정 → V₁, V₂, h, xc 자동 계산 (소수점 2자리)
  - 굴절파 구간 스테이션 2개 미만 시 "V₁만 계산됨" 안내
- 수동 모드: V₁·V₂ 선 핸들 드래그 → 실시간 속도 계산
- 결과 CSV 저장 (station_id, dist_m, pick_iso, travel_ms, time_offset_ms)
...
- **색상**: Seismic jet 컬러맵 (검정→파랑→시안→녹색→노랑→주황→빨강)
...
- **알고리즘**: `PSD[b] = 2 × |FFT_b|² / (sr × Σhann²)` → `dB = 10 × log₁₀(PSD)` (Hann 에너지 보정, 단측 스펙트럼)
- **표시**: 주파수(로그 스케일, 0.2~50 Hz) vs 파워 (Y축 오토스케일)
- **밀도 히트맵**: 누적 윈도우를 픽셀별 카운트로 집계 → 로그 정규화 → 파랑(희소)→빨강(밀집) 컬러맵 (ObsPy PPSD 스타일)
- **평균선**: 흰색 실선(1px)으로 전체 누적 평균 표시
- **실시간/누적 토글**: 실시간 모드는 최신 FFT 윈도우 청록 라인, 누적 모드는 밀도 히트맵 + 평균선
- **활용**: 배경 노이즈 수준 평가, 주파수 성분 분포 확인
...
- **표시**: 주파수(로그 스케일, 0.5~50 Hz) vs H/V 비율 (Y축 오토스케일)
- **f₀ 탐지**: 1~10 Hz 범위 H/V 최대값 → 수직 점선 + 레이블 표시 (H/V > 1.5 기준)
- **스무딩**: 적용 없음 — 윈도우 수 확보(FILE_HOP=128, 87.5% 오버랩)로 자연스러운 평활화
- **신뢰도**: 50 윈도우 미만 시 경고 표시 (약 10분 이상 측정 권장), 정상성 필터 적용 (SESAME 준수)
...
│       ├── spectrogram.js  # 가로 워터폴 스펙트로그램 (FFT → Canvas, Seismic jet)
│       ├── psd.js          # Welch PSD 모듈 (밀도 히트맵, 실시간/누적 토글)
│       ├── hvsr.js         # Nakamura HVSR 모듈 (부지 공진 주파수, 스무딩 없음)
│       ├── export-image.js # PNG 내보내기 모듈 (ObsPy 스타일 헤더)
│       └── hodochron.js    # 주시곡선 Canvas 렌더러 v1.2 (자동 회귀 + 수동 선)
└── docs/
    ├── PRD.md
    ├── ARCHITECTURE.md
    └── TASK.md
...
- [x] Cycle 4 완료 — Activity 4: 주시곡선 (굴절법 탐사) (3탭: 파싱/거리·시간조정·픽킹/분석)
- [ ] Cycle 5 예정 — Activity 5: 진원 찾기
