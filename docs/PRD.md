...
> 최종 업데이트: 2026-03-10 (Cycle 3 완료 기준)
...
| 캘리브레이션 | 측정 시작 후 100샘플(≈1초) 평균을 기준값으로 설정, 이후 Δ값 출력 |
| 캘리브레이션 진행률 | 상태 메시지에 "기준값 설정 중... (43/100)" 실시간 표시 |
| 센서 소스 표시 | 측정 시작 후 어떤 API를 사용 중인지 상태 메시지로 안내 |
| 배경색 시각화 | \|acc_z\| → 배경색 매핑: 0 → 파랑(hue 240), MAX_Z(1.5 m/s²) → 빨강(hue 0) |
| 실시간 파형 그래프 | Z축 슬라이딩 윈도우 10초, Quadratic Bezier 곡선 렌더링 |
| 자동 Y축 스케일 | 즉시 확장 / 천천히 축소 (decay 0.997), 최소 범위 ±0.5 m/s² |
| 시간 축 레이블 | 파형 하단 2초 간격 눈금 표시 (-10s ~ 0s 상대 시간) |
| 가속도 수치 표시 | 소수점 6자리 실시간 표시 (Firefox: −0.003421, Chrome: 0.100000) |
| 1차/2차 정지 | 1차: 센서 중단·그래프 유지 / 2차: 전체 초기화 |
...
| FFT 파라미터 | FFT_SIZE=256, HOP_SIZE=26 (~90% overlap, ObsPy 기준), MAX_FREQ=25Hz |
| 컬러맵 | Viridis-style LUT (어두운 보라 → 파랑 → 청록 → 초록 → 노랑) |
| 진폭 스케일 | log₁₀ 진폭: -3 (어둠, 0.001 m/s²) ~ -1 (밝음, 0.1 m/s²) |
| 스크롤 동기화 | 스펙트로그램 colWidth 동적 계산으로 파형 10초 윈도우와 동기 |
| 피크 주파수 표시 | 현재 프레임에서 가장 강한 주파수 Hz 실시간 표시 |
| CSV 전처리 | 전체 컬럼 사전 계산 후 requestAnimationFrame 7초 애니메이션 재생 |
| 파일 정보 표시 | 파일명, 길이(M:SS), 샘플 수, 샘플레이트 |
| 주파수 축 레이블 | 0/5/10/15/20/25Hz 오버레이 표시 |
...
| 코드 규모 | 파일당 300줄 이하, 빌드 도구 없음, 외부 라이브러리 없음 |
