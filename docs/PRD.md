...
> 최종 업데이트: 2026-04-04 (psd.js v3.0 다중 윈도우 + Peterson 참조선 + X축 범위 통일)
...
#### PSD — 전력 스펙트럼 밀도 (psd.js v3.1)

| 항목 | 내용 |
|------|------|
| 알고리즘 | Welch's method — `PSD[b] = 2 × |FFT_b|² / (sr × Σhann²)` (Hann 에너지 보정, 단측 스펙트럼 ×2) |
| 단위 | (m/s²)²/Hz → dB 변환: `10 × log₁₀(PSD)` |
| 렌더링 | 개별 윈도우 PSD 곡선(반투명 빨간 선) + Welch 평균선(청록, 굵음) 2층 표시 |
| 누적 | 모든 윈도우 _windows[]에 저장 — 측정이 쌓일수록 분포가 채워짐 |
| 파일 모드 | FILE_HOP=512 (50% 오버랩) — 10분 CSV ≈ 117개 윈도우 |
...
| X축 | 주파수 로그 스케일, **0.2~50Hz** (HVSR과 통일) |
| Peterson | NLNM/NHNM 참조선 내장 (SEIZMO 검증 계수) — "참조선 표시" 버튼으로 토글 |
| 설명 버튼 | ? 버튼 → 알고리즘·단위·활용 설명 모달 팝업 |

#### HVSR — 수평/수직 스펙트럼 비율 (hvsr.js v2.0)

| 항목 | 내용 |
|------|------|
| 알고리즘 | Nakamura(1989) + SESAME 2004 가이드라인 준수 |
| 공식 | `H(f) = √((|FFT_x|²+|FFT_y|²)/2)`, `HVSR(f) = mean[H(f)/V(f)]` per-window |
| 평균 방식 | **per-window H/V 비율 계산 후 평균** (SESAME-correct, 구 버전: 파워 누적 후 비율) |
| 센서 모드 | ±2-bin MA 스무딩, ~97.5% 오버랩 |
| 파일 모드 | Konno-Ohmachi 스무딩(b=40), 50% 오버랩, 정상성 필터 적용 |
| 정상성 필터 | RMS가 [0.5×, 2×]×중앙값 범위 벗어나는 윈도우 제거 |
| f₀ 탐색 | 0.2–20Hz, H/V ≥ 2.0 시 피크 마커 표시 |
| Y축 | 오토스케일(_hvDispMax): 데이터 최대값 기반 자동 조정 |
| 표시 | X축=주파수(로그, 0.2~50Hz), Y축=H/V 비율 |
...
|------|------|
| 트리거 | 분석 탭 우측 `?` 버튼 클릭 |
| 동작 | 현재 활성 분석 탭(스펙트로그램·PSD·HVSR)에 맞는 설명 모달 팝업 |
| 닫기 | 닫기 버튼 또는 배경 클릭 |
| 적용 범위 | 센서 모드·파일 모드 공통 (각 탭 세트에 ? 버튼 1개씩) |

#### 파일 모드 절대 시각 표시

| 항목 | 내용 |
|------|------|
| 기능 | CSV timestamp 기반 실제 측정 시각(HH:MM:SS) 표시 |
| 적용 위치 | 파형 X축 레이블 + 구간 선택 안내문 |
| 로직 | `_fileStartTime` (rows[0] epoch ms) + 구간 오프셋(초) → `fmtAbsTime()` |
| 폴백 | timestamp 없는 CSV → 파일 시작 기준 상대 시간(`fmtTime()`)으로 자동 전환 |
